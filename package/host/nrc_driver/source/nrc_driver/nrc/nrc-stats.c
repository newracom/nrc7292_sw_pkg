...
struct moving_average {
	...
	int max;
	int index;
	int (*compute)(uint8_t *arr, int index, int count);
	uint8_t arr[];
};
...
static int snr_compute(uint8_t *arr, int index, int count)
{
	int i;
	uint8_t min = U8_MAX;
	uint8_t max = 0;
...
static int rssi_compute(uint8_t *arr, int index, int count)
{
	int i;
	int8_t min = S8_MAX;
	int8_t max = S8_MIN;
