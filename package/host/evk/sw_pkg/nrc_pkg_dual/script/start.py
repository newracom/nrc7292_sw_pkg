...
def copyConf():
    os.system("sudo ~/nrc_pkg/sw/firmware/copy " + str(model))
    if strSTA() == 'AP':
        os.system("sudo cp ~/nrc_pkg/etc/dhcpcd/dhcpcd_ap.conf /etc/dhcpcd.conf")
        os.system("sudo cp ~/nrc_pkg/etc/dhcpcd/dnsmasq/dnsmasq_ap.conf /etc/dnsmasq.conf")
        if int(enable_11n) and role_11n == 'STA':
            print ("copy dhcpcd_ap_sta.conf &  dnsmasq_ap_sta.conf")
            os.system("sudo cp ~/nrc_pkg/etc/dhcpcd/dhcpcd_ap_sta.conf /etc/dhcpcd.conf")
            os.system("sudo cp ~/nrc_pkg/etc/dhcpcd/dnsmasq/dnsmasq_ap_sta.conf /etc/dnsmasq.conf")
    else:
        os.system("sudo cp ~/nrc_pkg/etc/dhcpcd/dhcpcd_sta.conf /etc/dhcpcd.conf")
        if int(enable_11n) and role_11n == 'AP':
            print ("copy dhcpcd_sta_ap.conf & dnsmasq_sta_ap.conf")
            os.system("sudo cp ~/nrc_pkg/etc/dhcpcd/dhcpcd_sta_ap.conf /etc/dhcpcd.conf")
            os.system("sudo cp ~/nrc_pkg/etc/dhcpcd/dnsmasq/dnsmasq_sta_ap.conf /etc/dnsmasq.conf")
...
def run_common():
    if int(max_cpuclock) == 1:
        ...
        os.system("sudo ~/nrc_pkg/script/fix_clock.sh")
    ...
    print "sudo insmod ~/nrc_pkg/sw/driver/nrc.ko " + insmod_arg
    os.system("sudo insmod ~/nrc_pkg/sw/driver/nrc.ko " + insmod_arg + "")
    time.sleep(5)
    ...
    os.system('python ~/nrc_pkg/etc/python/shell.py run --cmd="nrf txpwr ' + str(txpwr_val) + '"')

    print "[4] Set aggregation number"
    if maxagg_num:
        os.system('python ~/nrc_pkg/etc/python/test_send_addba.py 0')
        os.system('python ~/nrc_pkg/etc/python/shell.py run --cmd="set maxagg 1 ' + str(maxagg_num) + '"')
        time.sleep(1)


    print "[5] Set guard interval"
    os.system('python ~/nrc_pkg/etc/python/shell.py run --cmd="set gi ' + guard_int + '"')
    time.sleep(1)
...
def run_sta(interface):
    ...
    if strSecurity() == 'OPEN':
        os.system("sudo wpa_supplicant -i" + interface + " -c ~/nrc_pkg/script/conf/" + country + "/sta_halow_open.conf " + debug + " &")
    elif strSecurity() == 'WPA2-PSK':
        os.system("sudo wpa_supplicant -i" + interface + " -c ~/nrc_pkg/script/conf/" + country + "/sta_halow_wpa2.conf " + debug + " &")
    elif strSecurity() == 'WPA3-OWE':
        os.system("sudo wpa_supplicant -i" + interface + " -c ~/nrc_pkg/script/conf/" + country + "/sta_halow_owe.conf " + debug + " &")
    elif strSecurity() == 'WPA3-SAE':
        os.system("sudo wpa_supplicant -i" + interface + " -c ~/nrc_pkg/script/conf/" + country + "/sta_halow_sae.conf " + debug + " &")
    time.sleep(3)
...
def run_ap(interface):
    ...
    if strSecurity() == 'OPEN':
        os.system("sudo hostapd ~/nrc_pkg/script/conf/" + country + "/ap_halow_open.conf " + debug +" &")
    elif strSecurity() == 'WPA2-PSK':
        os.system("sudo hostapd ~/nrc_pkg/script/conf/" + country + "/ap_halow_wpa2.conf " + debug + "  &")
    elif strSecurity() == 'WPA3-OWE':
        os.system("sudo hostapd ~/nrc_pkg/script/conf/" + country + "/ap_halow_owe.conf " + debug + "  &")
    elif strSecurity() == 'WPA3-SAE':
        os.system("sudo hostapd ~/nrc_pkg/script/conf/" + country + "/ap_halow_sae.conf " + debug + "  &")
    time.sleep(3)
...
def run_11n(interface):
    ...
    if role_11n == 'AP':
        print "[*] Start 11n AP"
        os.system("sudo hostapd ~/nrc_pkg/script/conf/11N/hostapd_" + strSTA() + ".conf &")
        time.sleep(3)
        if strSTA() == 'STA':
        ...
        print "[*] Start 11n STA"
        os.system("sudo wpa_supplicant -i " + interface + " -c ~/nrc_pkg/script/conf/11N/wpa_supplicant.conf &")
        time.sleep(3)
        if strSTA() == 'AP':
            os.system('sudo sh -c "echo 1 > /proc/sys/net/ipv4/ip_forward"')
