				 ...
				 struct ieee80211_vif *vif)
{
	...
	nrc_dbg(NRC_DBG_MAC, "%s: VIF%d's hwqueue:", __func__, i_vif->index);
#endif
...
uint16_t get_base_freq(void)
{
	...
	if (nrc_cc[0] == 'U' && nrc_cc[1] == 'S') {
		...
		ret_freq = (kr_band == 0) ? K0_BASE_FREQ : (kr_band == 1) ? K1_BASE_FREQ : K2_BASE_FREQ;
...
#endif
{
	 ...
	 * nrc_cc can be changed to EU, K0, K1 or K2 in nrc_read_bd_tx_pwr().
	 */
				...
				u8 subcmd, const void *data, int data_len)
{
	...
	/* Update beacon */
	if ((subcmd >= NRC_SUBCMD_BCAST_FOTA_1) && (subcmd <= NRC_SUBCMD_BCAST_FOTA_4))
		return 0;
	return nrc_vendor_update_beacon(hw, vif);
}
